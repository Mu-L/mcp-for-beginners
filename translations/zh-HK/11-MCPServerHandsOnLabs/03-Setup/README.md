...
本實作實驗室將引導你完成建立具備 PostgreSQL 整合的 MCP 伺服器完整開發環境。你將配置所有必要工具、部署 Azure 資源，並在實作前驗證設定。

## 概述

完善的開發環境是成功開發 MCP 伺服器的關鍵。本實驗室提供逐步說明，涵蓋 Docker、Azure 服務、開發工具的設定，並驗證它們能正確協同運作。

完成本實驗室後，你將擁有完整且可用的開發環境，準備建構 Zava Retail MCP 伺服器。

## 學習目標

完成本實驗室後，你將能：

- <strong>安裝與配置</strong> 所有必備開發工具
- **部署 Azure 資源** ，支援 MCP 伺服器
- **設定 Docker 容器** ，包含 PostgreSQL 與 MCP 伺服器
- <strong>驗證</strong> 環境設定並測試連線
- <strong>排解</strong> 常見設定問題與錯誤
- <strong>理解</strong> 開發流程與檔案結構
...
### 必要知識
- 基本命令列使用（Windows 命令提示字元 / PowerShell）
- 了解環境變數
- Git 版本控制的熟悉度
- 基本 Docker 概念（容器、映像檔、資料卷）

### 系統需求
- <strong>作業系統</strong>：Windows 10/11、macOS 或 Linux
- <strong>記憶體</strong>：至少 8GB（建議 16GB）
- <strong>儲存空間</strong>：至少 10GB 可用空間
- <strong>網路</strong>：有網際網路連線以下載及部署 Azure

### 帳戶需求
- **Azure 訂閱**：免費方案可用
- **GitHub 帳戶**：存取程式碼庫用
- **Docker Hub 帳戶**：（選擇性）自訂映像檔發佈用
...
2. <strong>安裝與配置</strong>：
   - 以管理員身份執行安裝程式
   - 出現提示時啟用 WSL 2 整合
   - 安裝完成後重新啟動電腦

3. <strong>驗證安裝</strong>：
   ```cmd
   docker --version
...
```bash
# Git 通常已預先安裝，但你可以透過 Homebrew 更新
brew install git
```
...
Visual Studio Code 提供整合開發環境，支援 MCP 開發。
...
| <strong>資源</strong> | <strong>用途</strong> | <strong>估計費用</strong> |
|--------------|-------------|-------------------|
...
3. **測試 MCP 伺服器連線**：
   - 在 AI Chat 輸入 `#zava` 並選擇其中一個配置伺服器
   - 詢問：「資料庫中有哪些資料表？」
   - 你應該會收到列出零售資料庫表的回覆
...
**✅ 基本工具**
- [ ] 安裝並執行 Docker 20.10 以上版本
- [ ] 安裝並認證 Azure CLI 2.40 以上
- [ ] 安裝 Python 3.8 以上版本並具 pip
- [ ] 安裝 Git 2.30 以上版本
- [ ] VS Code 與必要擴充套件
...
**✅ 環境配置**
- [ ] 已建立包含完整變數 `.env` 檔案
- [ ] Azure 憑證正常（以 `az account show` 測試）
- [ ] PostgreSQL 容器啟動且可存取
- [ ] 資料庫載入範例資料
...
✅ <strong>完整開發環境</strong>：所有工具安裝配置齊全  
✅ **Azure 資源部署**：AI 服務及支援基礎設施  
✅ **運行中的 Docker 環境**：PostgreSQL 與 MCP 伺服器容器  
✅ **VS Code 整合**：MCP 伺服器設定與存取  
✅ <strong>驗證環境</strong>：所有元件經過測試協同運作  
✅ <strong>排解知識</strong>：了解常見問題與解決方案  

## 🚀 下一步

環境就緒後，繼續前往 **[Lab 04: 資料庫設計與結構](../04-Database/README.md)**：

- 深入探討零售資料庫結構
- 了解多租戶資料建模
- 學習列級安全 (Row Level Security) 實作
- 操作零售範例資料

## 📚 額外資源

### 開發工具
- [Docker 文件](https://docs.docker.com/) - 完整 Docker 參考
- [Azure CLI 參考](https://docs.microsoft.com/cli/azure/) - Azure CLI 命令
- [VS Code 文件](https://code.visualstudio.com/docs) - 編輯器配置與擴充套件
...
<strong>下一步</strong>：環境已準備好？繼續瀏覽 [Lab 04: 資料庫設計與結構](../04-Database/README.md)
