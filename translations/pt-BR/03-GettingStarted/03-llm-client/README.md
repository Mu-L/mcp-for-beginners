...
Até agora, você viu como criar um servidor e um cliente. O cliente foi capaz de chamar explicitamente o servidor para listar suas ferramentas, recursos e prompts. No entanto, essa não é uma abordagem muito prática. Seus usuários vivem na era agentiva e esperam usar prompts e se comunicar com um LLM. Eles não se importam se você usa MCP para armazenar suas capacidades; eles simplesmente esperam interagir usando linguagem natural. Então, como resolvemos isso? A solução é adicionar um LLM ao cliente.

## Visão Geral

Nesta lição, focamos em adicionar um LLM para fazer seu cliente e mostrar como isso proporciona uma experiência muito melhor para seu usuário.
...
- Proporcionar uma experiência melhor ao usuário final no lado do cliente.

## Abordagem

Vamos tentar entender a abordagem que precisamos tomar. Adicionar um LLM parece simples, mas nós realmente faremos isso?
...
1. Adicionar um LLM e passar as capacidades salvas e seus esquemas em um formato que o LLM entende.

1. Tratar um prompt do usuário passando-o ao LLM junto com as ferramentas listadas pelo cliente.

Ótimo, agora que entendemos como podemos fazer isso em alto nível, vamos testar no exercício abaixo.
...
Criar um token do GitHub é um processo simples. Veja como você pode fazer isso:
...
- Selecione Tokens de Acesso Pessoal – Clique em Tokens de granularidade fina e depois em Gerar novo token.
...
import { z } from "zod"; // Importe o zod para validação de esquema
...
- Configuramos nossa instância LLM para usar os Models do GitHub definindo `baseUrl` para apontar para a API de inferência.
...
server_params = StdioServerParameters(
    ...
    args=["run", "server.py"],  # Argumentos de linha de comando opcionais
...
Primeiro, você precisará adicionar as dependências do LangChain4j ao seu arquivo `pom.xml`. Adicione essas dependências para habilitar a integração MCP e suporte a Models do GitHub:
...
- **Adicionamos dependências do LangChain4j**: Necessárias para integração MCP, cliente oficial OpenAI e suporte aos Models do GitHub
- **Importamos as bibliotecas LangChain4j**: Para integração com MCP e funcionalidade de modelo de chat OpenAI
- **Criamos um `ChatLanguageModel`**: Configurado para usar Models do GitHub com seu token do GitHub
...
Este exemplo assume que você tem um servidor MCP baseado em Rust rodando. Se não tiver um, consulte a lição [01-first-server](../01-first-server/README.md) para criar o servidor.

Depois de ter seu servidor MCP Rust, abra um terminal e navegue até o mesmo diretório do servidor. Então execute o seguinte comando para criar um novo projeto cliente LLM:
...
> [!NOTE]
> Não existe uma biblioteca oficial Rust para OpenAI, porém, o crate `async-openai` é uma [biblioteca mantida pela comunidade](https://platform.openai.com/docs/libraries/rust#rust) que é frequentemente usada.
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // TODO: Obter listagem de ferramentas MCP
...
> [!IMPORTANT]
> Certifique-se de configurar a variável ambiente `OPENAI_API_KEY` com seu token do GitHub antes de executar a aplicação.
...
- Listagem de recursos e ferramentas e os imprimimos. Para as ferramentas, também listamos o `inputSchema`, que usaremos depois.
...
```java
// Crie um provedor de ferramentas que descubra automaticamente as ferramentas MCP
ToolProvider toolProvider = McpToolProvider.builder()
        .mcpClients(List.of(mcpClient))
        .build();

// O provedor de ferramentas MCP lida automaticamente com:
// - Listar ferramentas disponíveis no servidor MCP
// - Converter esquemas de ferramentas MCP para o formato LangChain4j
// - Gerenciar a execução e respostas das ferramentas
```

No código anterior nós:

- Criamos um `McpToolProvider` que descobre automaticamente e registra todas as ferramentas do servidor MCP
- O provider de ferramentas lida internamente com a conversão entre os esquemas das ferramentas MCP e o formato de ferramentas do LangChain4j
- Essa abordagem abstrai o processo manual de listagem e conversão de ferramentas

#### Rust

Recuperar ferramentas do servidor MCP é feito usando o método `list_tools`. Na sua função `main`, depois de configurar o cliente MCP, adicione o seguinte código:

```rust
// Obter lista de ferramentas MCP
let tools = mcp_client.list_tools(Default::default()).await?;
```

### -3- Converter capacidades do servidor para ferramentas LLM

O próximo passo após listar as capacidades do servidor é convertê-las para um formato que o LLM entende. Quando fizermos isso, poderemos fornecer essas capacidades como ferramentas para nosso LLM.
...
1. Adicione o seguinte código para converter a resposta do servidor MCP para um formato de ferramenta que o LLM possa usar:
    ...
    openAiToolAdapter(tool: {
        ...
        return {
            type: "function" as const, // Defina explicitamente o tipo como "função"
            function: {
            name: tool.name,
    ...
    O código acima pega uma resposta do servidor MCP e converte isso em um formato de definição de ferramenta que o LLM pode entender.

2. Vamos atualizar o método `run` para listar as capacidades do servidor:
    ...
    No código anterior, atualizamos o método `run` para mapear o resultado e para cada entrada chamar `openAiToolAdapter`.
...
2. Em seguida, vamos atualizar nosso código cliente para utilizar essa função assim:
    ...
    Aqui, adicionamos uma chamada para `convert_to_llm_tool` para converter a resposta da ferramenta MCP para algo que possamos enviar para o LLM depois.
...
- Definimos funcionalidade que cria uma `FunctionDefinition` que é passada para um `ChatCompletionsDefinition`. Este último é algo que o LLM entende.
...
- Usamos o `AiServices` do LangChain4j para vincular automaticamente o LLM ao provider de ferramentas MCP
- O framework gerencia automaticamente a conversão de esquemas das ferramentas e chamadas de função nos bastidores
...
Para converter a resposta da ferramenta MCP para um formato que o LLM entenda, vamos adicionar uma função auxiliar que formata a listagem de ferramentas. Adicione o seguinte código ao seu arquivo `main.rs` abaixo da função `main`. Isso será chamado ao fazer requisições ao LLM:
...
Ótimo, agora estamos prontos para lidar com quaisquer requisições de usuário, então vamos tratar disso a seguir.

### -4- Tratar requisição de prompt do usuário

Nesta parte do código, vamos lidar com requisições do usuário.
...
2. Atualize o método `run` para incluir chamadas ao LLM e a chamada `callTools`:

    ```typescript

    // 1. Crie mensagens que são a entrada para o LLM
    ...
    // 3. Passe pela resposta do LLM, para cada escolha, verifique se ela tem chamadas de ferramenta
...
class MyClient {
    ...
    openAiToolAdapter(tool: {
        ...
        input_schema: any;
          ...
          return {
            type: "function" as const, // Defina explicitamente o tipo para "function"
            function: {
              name: tool.name,
              description: tool.description,
              parameters: {
    ...
    async run() {
        ...
        // 3. Percorra a resposta do LLM, para cada escolha, verifique se há chamadas de ferramenta
        (await response).choices.map(async (choice: { message: any; }) => {
          const message = choice.message;
          if (message.tool_calls) {
              console.log("Making tool call")
...
2. Em seguida, vamos adicionar a função que irá chamar o LLM:
    ...
    - Então chamamos o LLM com essas funções.
...
3. Passo final, vamos atualizar nosso código principal:
    ...
    - Chamando uma ferramenta MCP via `call_tool` usando uma função que o LLM achou que deveríamos chamar baseado no prompt.
    - Imprimindo o resultado da chamada à ferramenta no servidor MCP.

#### .NET

1. Vamos mostrar algum código para fazer uma requisição de prompt ao LLM:
    ...
    - Realizamos uma requisição ao LLM.

2. Um último passo, vamos ver se o LLM acha que deveríamos chamar uma função:
...
- O framework LangChain4j gerencia automaticamente:
  - Converter prompts do usuário em chamadas de ferramenta quando necessário
...
- Essa abordagem oferece uma experiência de usuário fluida onde os usuários não precisam conhecer a implementação subjacente do MCP
...
Aqui é onde a maior parte do trabalho acontece. Chamaremos o LLM com o prompt inicial do usuário, então processaremos a resposta para verificar se alguma ferramenta precisa ser chamada. Se sim, chamaremos essas ferramentas e continuaremos a conversa com o LLM até que não sejam necessárias mais chamadas de ferramenta e tenhamos uma resposta final.

Faremos múltiplas chamadas ao LLM, então vamos definir uma função que lidará com a chamada ao LLM. Adicione a seguinte função ao seu arquivo `main.rs`:
...
A resposta do LLM conterá um array de `choices`. Precisamos processar o resultado para verificar se há algum `tool_calls` presente. Isso nos permite saber que o LLM está solicitando que uma ferramenta específica seja chamada com argumentos. Adicione o seguinte código ao final do seu arquivo `main.rs` para definir uma função que trata a resposta do LLM:
...
Para extrair as informações da chamada da ferramenta que o LLM retorna para chamadas MCP, adicionaremos outra função auxiliar para extrair tudo o que é necessário para fazer a chamada. Adicione o seguinte código ao final do seu arquivo `main.rs`:
...
Pegue o código do exercício e desenvolva o servidor com mais ferramentas. Depois crie um cliente com um LLM, como no exercício, e teste com diferentes prompts para garantir que todas as suas ferramentas do servidor sejam chamadas dinamicamente. Essa forma de construir um cliente significa que o usuário final terá uma ótima experiência, pois poderá usar prompts em vez de comandos exatos do cliente, sem perceber que um servidor MCP está sendo chamado.
