...
Dette er en eksempel MCP Server i Python som implementerer værverktøy med simulerte svar. Den kan brukes som en ramme for din egen MCP Server. Den inkluderer følgende funksjoner:

- **Weather Tool**: Et verktøy som gir simulert værinformasjon basert på gitt sted.
...
Det finnes to tilnærminger for å sette opp miljøet for dette prosjektet. Du kan velge den som passer best for deg.

> Merk: Last inn VSCode eller terminal på nytt for å sikre at python i det virtuelle miljøet brukes etter opprettelse av det virtuelle miljøet.
...
1. Åpne debug-panelet i VS Code. Velg `Debug in Agent Builder` eller trykk `F5` for å starte feilsøking av MCP-serveren.  
2. Bruk Microsoft Foundry Toolkit Agent Builder for å teste serveren med [denne prompten](../../../../../../../../../../../open_prompt_builder). Serveren kobles automatisk til Agent Builder.  
...
| `.aitk`     | Konfigurasjoner for Microsoft Foundry Toolkit |
...
> - Alle feilsøkingsmoduser støtter breakpoint, slik at du kan legge til breakpoints i verktøyets implementasjonskode.

| Feilsøkingsmodus | Beskrivelse | Steg for feilsøking |
| ---------------- | ----------- | ------------------- |
| Agent Builder | Feilsøk MCP-serveren i Agent Builder via Microsoft Foundry Toolkit. | 1. Åpne debug-panelet i VS Code. Velg `Debug in Agent Builder` og trykk `F5` for å starte feilsøking av MCP-serveren.<br>2. Bruk Microsoft Foundry Toolkit Agent Builder for å teste serveren med [denne prompten](../../../../../../../../../../../open_prompt_builder). Serveren kobles automatisk til Agent Builder.<br>3. Klikk `Run` for å teste serveren med prompten. |
| MCP Inspector | Feilsøk MCP-serveren ved hjelp av MCP Inspector. | 1. Installer [Node.js](https://nodejs.org/)<br> 2. Sett opp Inspector: `cd inspector` && `npm install`<br> 3. Åpne debug-panelet i VS Code. Velg `Debug SSE in Inspector (Edge)` eller `Debug SSE in Inspector (Chrome)`. Trykk F5 for å starte feilsøking.<br> 4. Når MCP Inspector starter i nettleseren, trykk på `Connect`-knappen for å koble til denne MCP-serveren.<br> 5. Deretter kan du `List Tools`, velge et verktøy, skrive inn parametere, og `Run Tool` for å feilsøke serverkoden.<br> |

## Standardporter og tilpasninger

| Feilsøkingsmodus | Porter | Definisjoner | Tilpasninger | Merk |
| ---------------- | ------ | ------------ | ------------ | ---- |
...
Hvis du har tilbakemeldinger eller forslag til denne malen, vennligst åpne en sak på [Microsoft Foundry Toolkit GitHub-repositoriet](https://github.com/microsoft/vscode-ai-toolkit/issues)
