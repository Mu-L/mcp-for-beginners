...
2. `run` method ကို Server စွမ်းရည်များ စာရင်းထုတ်ရန် ပြင်ဆင်ပါ။
...
2. Client code ကို အောက်ပါအတိုင်း ပြင်ဆင်ပါ။
...
2. ရှိပြီးသား code ကို ဒီ function ဖြင့် အသုံးချခြင်း -
...
2. `run` method ကို LLM ခေါ်ယူခြင်းနှင့် `callTools` သုံးခြင်း ပါအောင် ပြင်ဆင်ပါ။
...
class MyClient {
    ...
    async run() {
        ...
        // ၃။ LLM အၿပန္အလွန္ကို ဖြတ္သန္းၿပီး၊ တစ္ခုခ်င္းစီအတြက္ ကိရိယာခ့ံခ်ိတ္မှု ရွိမရွိ စစ္ေဆးပါ
        (await response).choices.map(async (choice: { message: any; }) => {
          const message = choice.message;
          if (message.tool_calls) {
              console.log("Making tool call")
...
2. LLM ခေါ်မည့် function ကို ထည့်ပါ။
...
3. နောက်ဆုံးအဆင့် Client ရဲ့ main code ကို ပြင်ဆင်ပါ။
...
2. နောက်ဆုံးအဆင့် LLM မှ function call ထားသလား စစ်ဆေးခြင်း -
