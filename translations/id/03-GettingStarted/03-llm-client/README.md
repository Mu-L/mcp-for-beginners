...
1. Menambahkan LLM dan mengoper kapabilitas yang disimpan beserta skemanya dalam format yang dipahami LLM.

1. Menangani prompt pengguna dengan mengoperkannya ke LLM bersama dengan alat yang didaftarkan oleh klien.
...
- Mengonfigurasi instance LLM kami untuk menggunakan Models GitHub dengan mengatur `baseUrl` mengarah ke API inferensi.
...
- **Membuat `ChatLanguageModel`**: Dikofnigurasi menggunakan Models GitHub dengan token GitHub Anda
...
Contoh ini mengasumsikan Anda memiliki server MCP berbasis Rust yang berjalan. Jika Anda belum memiliki satu, rujuk kembali ke pelajaran [01-first-server](../01-first-server/README.md) untuk membuat server.
...
> [!NOTE]
> Tidak ada perpustakaan Rust resmi untuk OpenAI, namun, crate `async-openai` adalah [perpustakaan yang dikelola komunitas](https://platform.openai.com/docs/libraries/rust#rust) yang umum digunakan.
...
> [!IMPORTANT]
> Pastikan untuk menetapkan variabel lingkungan `OPENAI_API_KEY` dengan token GitHub Anda sebelum menjalankan aplikasi.
...
- Membuat metode `run` yang bertanggung jawab menangani alur aplikasi kami. Sejauh ini hanya mendaftar alat tetapi akan kami tambahkan lagi sebentar lagi.
...
1. Tambahkan kode berikut untuk mengonversi respons dari Server MCP ke format alat yang dapat digunakan oleh LLM:
    ...
    openAiToolAdapter(tool: {
        ...
        return {
            type: "function" as const, // Secara eksplisit tetapkan tipe ke "function"
            function: {
            name: tool.name,
    ...
    Kode di atas mengambil respons dari Server MCP dan mengonversinya ke format definisi alat yang dipahami oleh LLM.

2. Mari kita perbarui metode `run` selanjutnya untuk mendaftar kapabilitas server:
    ...
    Dalam kode sebelumnya, kami memperbarui metode `run` untuk memetakan hasil dan untuk setiap entri memanggil `openAiToolAdapter`.
...
2. Selanjutnya, mari kita perbarui kode klien kita untuk memanfaatkan fungsi ini seperti berikut:
    ...
    Di sini, kita menambahkan panggilan ke `convert_to_llm_tool` untuk mengonversi respons alat MCP ke sesuatu yang bisa kita berikan ke LLM nanti.

#### .NET

1. Mari kita tambahkan kode untuk mengonversi respons alat MCP ke sesuatu yang dipahami LLM
...
- Mendefinisikan fungsionalitas yang membuat FunctionDefinition yang diteruskan ke ChatCompletionsDefinition. Yang terakhir ini adalah sesuatu yang dipahami oleh LLM.

2. Mari kita lihat bagaimana kita dapat memperbarui beberapa kode yang ada untuk memanfaatkan fungsi di atas:
...
- Kerangka kerja secara otomatis menangani konversi skema alat dan pemanggilan fungsi di latar belakang
...
2. Perbarui metode `run` untuk memasukkan panggilan ke LLM dan memanggil `callTools`:

    ```typescript

    // 1. Membuat pesan yang menjadi input untuk LLM
    ...
    // 3. Melalui respons LLM, untuk setiap pilihan, periksa apakah ada panggilan alat
...
2. Selanjutnya, mari kita tambahkan fungsi yang akan memanggil LLM:
...
3. Langkah terakhir, mari kita perbarui kode utama kita:

    ```python
    prompt = "Add 2 to 20"

    # tanyakan pada LLM alat apa yang harus digunakan, jika ada
    ...
    Di sana, itu langkah terakhir, dalam kode di atas kami:

    - Memanggil alat MCP lewat `call_tool` menggunakan fungsi yang LLM pikir harus dipanggil berdasarkan prompt kita.
    - Mencetak hasil panggilan alat ke Server MCP.

#### .NET

1. Mari kita tunjukkan beberapa kode untuk melakukan permintaan prompt ke LLM:
...
2. Satu langkah terakhir, mari kita lihat apakah LLM pikir kita harus memanggil sebuah fungsi:
...
- Metode `bot.chat()` mengembalikan respons bahasa alami yang dapat menyertakan hasil eksekusi alat MCP
- Pendekatan ini menyediakan pengalaman pengguna yang mulus di mana pengguna tidak perlu tahu tentang implementasi MCP di balik layar
