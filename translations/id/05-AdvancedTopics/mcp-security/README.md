# Praktik Terbaik Keamanan MCP - Panduan Implementasi Lanjutan
...
### **Persyaratan Kritis dari Spesifikasi MCP (2025-11-25):**
...
Implementasi yang ditingkatkan memanfaatkan ekosistem keamanan Microsoft:
...
Implementasi Spring Security yang ditingkatkan mengikuti pola keamanan OAuth 2.1 yang diwajibkan oleh spesifikasi MCP:
...
public class AdvancedMcpSecurityConfig {
    ...
    public JwtDecoder jwtDecoder() {
        ...
        // WAJIB: Konfigurasi validasi audiens
    ...
    public Jwt validator jwtValidator() {
        ...
        // Validasi penerbit adalah Microsoft Entra ID
...
public class McpTokenValidator implements OAuth2TokenValidator<Jwt> {
    ...
    public OAuth2TokenValidatorResult validate(Jwt jwt) {
        ...
        // Validasi klaim yang diperlukan untuk akses MCP
        if (!hasRequiredScopes(jwt)) {
            errors.add(new OAuth2Error("invalid_scope", 
                "Token missing required MCP scopes", null));
        }
        
        // Periksa indikator risiko tinggi
        if (hasRiskIndicators(jwt)) {
            errors.add(new OAuth2Error("high_risk_token", 
                "Token indicates high-risk authentication", null));
        }
        
        // Validasi pengikatan token jika ada
    ...
    private boolean hasRiskIndicators(Jwt jwt) {
        // Periksa indikator risiko Entra ID
...
public class AdvancedMcpSecurityInterceptor implements ToolExecutionInterceptor {
    ...
    public void beforeToolExecution(ToolRequest request, Authentication authentication) {
        ...
        try {
            // 1. Validasi audiens token (WAJIB)
            validateTokenAudience(authentication);
            
            // 2. Periksa upaya injeksi prompt
            ...
            // 3. Penyaringan keamanan konten menggunakan Azure Content Safety
    ...
    private boolean hasResourceAccess(String userId, String resourceId) {
        // Implementasi akan memeriksa izin sumber daya yang terperinci
...
class MicrosoftPromptShieldsIntegration:
    ...
    async def analyze_prompt_injection(self, text: str) -> Dict:
        ...
        try:
            # Gunakan Azure Content Safety untuk deteksi jailbreak
            response = await self.content_safety_client.analyze_text(
                ...
                output_type="FourSeverityLevels"  # Aman, Rendah, Sedang, Tinggi
            )
    ...
    async def apply_spotlighting(self, text: str, trusted_instructions: str) -> str:
        ...
        # Spotlighting membantu model AI membedakan antara instruksi sistem dan konten pengguna
...
class AdvancedPiiDetector:
    ...
    async def detect_pii_advanced(self, text: str, parameters: Dict) -> List[Dict]:
        ...
        # Deteksi berbasis regex standar
    ...
    async def analyze_contextual_pii(self, text: str, parameters: Dict) -> List[Dict]:
        ...
        # Periksa nama parameter untuk indikator PII
...
# Dekorator keamanan yang ditingkatkan dengan integrasi keamanan AI Microsoft
    ...
    def decorator(cls):
        ...
        async def secure_execute(self, request: ToolRequest):
            ...
            try:
                ...
                # 1. Validasi MFA (jika diperlukan)
                if require_mfa and not validate_mfa_token(request.context.get('token')):
                    raise SecurityException("Multi-factor authentication required")
                
                # 2. Deteksi Injeksi Prompt
                ...
                # 4. Deteksi dan Perlindungan PII
...
# Contoh implementasi dengan keamanan yang ditingkatkan
...
class EnterpriseCustomerDataTool(Tool):
    ...
    async def execute_async(self, request: ToolRequest):
        # Implementasi akan mengakses data pelanggan
        # Semua kontrol keamanan diterapkan melalui dekorator
...
async def validate_mfa_token(token: str) -> bool:
    ...
    # Implementasi akan memvalidasi token MFA dengan Entra ID
    return True  # Disederhanakan untuk contoh

async def analyze_content_safety(text: str, level: str) -> Dict:
    """Analyze content safety using Azure Content Safety"""
    # Implementasi akan memanggil API Azure Content Safety
...
## Mitigasi Ancaman Keamanan MCP Lanjutan
...
class AdvancedConfusedDeputyProtection:
    ...
    async def validate_dynamic_client_registration(
        ...
        try:
            ...
            # 4. Validasi hubungan ID klien statis
            if not await self.validate_static_client_relationship(static_client_id, client_id):
                self.logger.warning(f"Invalid static client relationship: {static_client_id} -> {client_id}")
                return False
            
            # Cache validasi yang berhasil
    ...
    async def validate_user_consent(
        ...
        try:
            # Dekode dan validasi token persetujuan
            consent_data = await self.decode_consent_token(consent_token)
            
            if not consent_data:
                return False
            
            # Verifikasi spesifisitas persetujuan
            expected_consent = {
                'client_id': client_id,
                'redirect_uri': redirect_uri,
    ...
    async def validate_redirect_uri(self, redirect_uri: str, client_id: str) -> bool:
        ...
        try:
            ...
            security_checks = [
                # Harus menggunakan HTTPS untuk keamanan
                parsed_uri.scheme == 'https',
                
                # Validasi domain
                await self.validate_domain_ownership(parsed_uri.netloc, client_id),
                
                # Tidak ada parameter kueri yang mencurigakan
    ...
    async def implement_pkce_validation(
        ...
        try:
            ...
            if code_challenge_method == "S256":
                # Buat tantangan kode dari verifier
...
async def secure_oauth_proxy_flow():
    ...
    async def handle_dynamic_client_registration(request):
        ...
        # Validasi WAJIB sesuai spesifikasi MCP
        ...
        # Lanjutkan dengan alur OAuth hanya setelah validasi
    ...
    async def handle_authorization_callback(request):
        ...
        # Validasi PKCE (WAJIB untuk OAuth 2.1)
...
### **2. Pencegahan Token Passthrough**
...
class TokenPassthroughPrevention:
    ...
    async def validate_token_for_mcp_server(self, token: str) -> Dict:
        ...
        try:
            ...
            # Dekode tanpa verifikasi terlebih dahulu untuk memeriksa klaim
            unverified_payload = jwt.decode(
                token, options={"verify_signature": False}
            )
            
            # 1. WAJIB: Validasi klaim audiens
            ...
            # 4. Sekarang verifikasi tanda tangan dengan validasi yang tepat
            # Ini akan menggunakan kunci publik penerbit
    ...
    async def prevent_token_passthrough(self, downstream_request: Dict) -> Dict:
        ...
        try:
            ...
            # Validasi token asli diterbitkan untuk server MCP ini
            ...
            # Perbarui permintaan dengan token baru
    ...
    async def issue_downstream_token(
        ...
        # Tanda tangani token dengan kunci privat server MCP
        return await self.sign_downstream_token(token_payload)
```

### **3. Pencegahan Pembajakan Sesi**

**Keamanan Sesi Lanjutan:**
...
class AdvancedSessionSecurity:
    ...
    async def generate_secure_session_id(self, user_id: str, additional_context: Dict = None) -> str:
        ...
        # Enkripsi ID sesi untuk keamanan tambahan
...
class EnterpriseSecurityMonitoring:
    ...
    async def log_mcp_security_event(self, event_data: Dict):
        ...
        with self.tracer.start_as_current_span("mcp_security_event") as span:
            # Tambahkan properti terstruktur ke span
            span.set_attributes({
    ...
    async def create_security_alert(self, event_data: Dict):
        ...
        # Kirim ke Azure Sentinel atau pusat operasi keamanan
        await self.send_to_security_center(alert_data)
    
    async def monitor_tool_usage_patterns(self, user_id: str, tool_name: str):
        """Monitor for unusual tool usage patterns that might indicate compromise"""
        
        # Dapatkan riwayat penggunaan terbaru
...
class MCPThreatDetectionPipeline:
    ...
    async def analyze_request_threat_level(self, request: Dict) -> Dict:
        ...
        # 5. Hitung skor risiko akhir dan rekomendasi
...
### **Integrasi Keamanan Rantai Pasokan**
...
class MCPSupplyChainSecurity:
    ...
    async def validate_mcp_component_security(self, component: Dict) -> Dict:
        ...
        try:
            # 1. Pemindaian Keamanan Lanjutan GitHub
